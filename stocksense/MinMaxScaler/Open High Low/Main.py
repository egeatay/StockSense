...
def main():
    ...
    # AMD = .02 AMZN = .03 NVDA = .035 TSLA = .06 [0.03,0.04,0.01,0.1]
    ...
    for stock, set_size, batch,size in zip(stocks, test_set_sizes,batch_size,sizes):
        ...
        X_train, y_train = Clean.cleaningLowHigh(data,cd,stockLocation,set_size)

        M.modelTrain(X_train,y_train,stockLocation,cd,batch,"HL",2)

        X_train, y_train = Clean.cleaning(data,cd,stockLocation,set_size)

        M.modelTrain(X_train,y_train,stockLocation,cd,batch,"Close",1)
